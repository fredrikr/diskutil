...
# diskutil.rb - a program for manipulating d81 and d64 disk images

# This is release 2. Find the latest version at 
# https://files.mega65.org?id=7e696492-f251-4c2e-b857-ffc965d8a869
...
# d81 disk images. If a disk image is then copied onto a real 
# floppy (using a full disk copier (e.g. BACKUP command in Basic 65), 
# not a file copier), and a program reads data from the floppy on 
# a MEGA65, this interleave scheme makes file reads about 5x faster, 
# compared to the default interleave scheme. To disable this scheme, 
# add -defint to command line.
...
#	attr_accessor :interleave_scheme
	...
	def read_file(filename)
		...
		if track < 1 or track > @tracks or sect < 0 or sect >= @track_length[track]
		...
		while !done
			...
			unless done
				...
				if next_track == 0
					...
					next_track = @dir_start[0]
					next_sect = (sect + @dir_interleave) % @track_length[next_track]
					tried_sectors = 0
					while sector_allocated?(next_track, next_sect)
						tried_sectors += 1
						next_sect = (next_sect + 1) % @track_length[next_track]
						raise "Directory full" if tried_sectors >= @track_length[next_track]
					end
					@contents[sectpos, 2] = [next_track, next_sect]
					allocate_sector(next_track, next_sect)
					@contents[(@track_offset[next_track] + next_sect) * 256, 256] = [0, 0xff] +
						Array.new(254, 0)
#					puts "Wrote new directory block #{next_track}, #{next_sect}"
				end
				track = next_track
				sect = next_sect
			end
...
class D64_image < Disk_image
	...
	def initialize(args)
		...
		if load
			...
			if @contents.length != 174848
				raise "Disk image '#{diskimage_filename}' isn't the correct length (should be 174848 bytes)"
			end
			...
			@contents[@track_offset[18] * 256, track1800.length] = track1800

			# Add a directory block
			@contents[(@track_offset[18] + 1) * 256, 256] = [0, 0xff] +
				Array.new(254, 0)
	...
	def verify_bam_for_track(track)
		offset = 4 * track  
		pos = @track_offset[18] * 256 + offset
		free = @contents[pos].ord
		c = 0
		3.times do |j|
			byte = @contents[pos + j + 1]
			8.times do
				c += 1 if byte & 1 == 1
				byte >>= 1
			end
		end
		raise "ERROR: Bad BAM for track #{track}" if c != free
	end

	def set_title(ascii_title)
		name = ascii_to_16_char_petscii(ascii_title,0xA0).unpack("C*")
		title_start = @track_offset[18] * 256 + 0x90
		@contents[title_start, 16] = name
	end

	def get_title
		title_start = @track_offset[18] * 256 + 0x90
		petscii_to_ascii(@contents[title_start, 16])
	end

	def set_id(ascii_id)
		id = ascii_to_petscii(ascii_id).unpack("C*")
		id_start = @track_offset[18] * 256 + 0xA2
		@contents[id_start, 2] = id
	end

	def get_id
		id_start = @track_offset[18] * 256 + 0xA2
		petscii_to_ascii(@contents[id_start, 2])
	end

	def get_dos
		dos_start = @track_offset[18] * 256 + 0xA5
		petscii_to_ascii(@contents[dos_start, 2])
	end

	def get_free_blocks
		sectpos = (@track_offset[@dir_start[0]]) * 256 + 4
		free = 0
		@tracks.times do |i|
			free += @contents[sectpos + 4 * i] if i != @dir_start[0] - 1
		end
		free
	end
...
#		puts "ALLOCATE: #{track},#{sector}"
		print "*" if $PRINT_DISK_MAP
		bam_start = @track_offset[18] * 256
		index1 = track * 4
		index2 = index1 + 1 + (sector / 8)

		# adjust number of free sectors

		free = @contents[bam_start + index1]
		if free < 1 or free > @track_length[track]
			puts "BAD FREE TRACK SPACE: #{track}, #{sector}"
		end
		
		@contents[bam_start + index1] = free - 1
		# allocate sector
		index3 = 255 - 2**(sector % 8)
		@contents[bam_start + index2] &= index3
	end
...
#		print "*" if $PRINT_DISK_MAP
	...
	def sector_allocated?(track, sector)
		...
		# is sector allocated?
		return @contents[bam_start + index2] & index3 == 0
	end
	...
	def find_free_file_start_sector(last_sector = nil)
		if last_sector
			...
			1.upto 17 do |t|
				@track_length[18 - t].times do |s|
					t1 = 18 - t
					unless sector_allocated?(t1, s)
						allocate_sector(t1, s)
						return [t1, s]
					end
					t2 = 18 + t
					unless s >= @track_length[t2] or sector_allocated?(t2, s)
						allocate_sector(t2, s)
						return [t2, s]
					end
				end
			end
	...
	def find_next_free_sector(track, sector)
		...
		tried_track_35 = nil
		tried_sectors = []
		next_sector = (sector + @interleave) % @track_length[track]
		z = 0
		loop do
			z += 1
			raise "ERROR: Failing to find free sector or note that disk is full!" if z > 700
			unless sector_allocated?(track, next_sector)
				allocate_sector(track, next_sector)
				return [track, next_sector]
#			else
#				print "A#{track},#{next_sector}"
			end
			tried_sectors.push next_sector unless tried_sectors.include? next_sector 
			if tried_sectors.length >= @track_length[track]
#				puts "TRACK #{track} FULL"
				# This track is full, go to next
				tried_track_1 = true if track == 1
				tried_track_35 = true if track == 35
				return nil if tried_track_1 and tried_track_35 # No free sectors, fail!
				...
				tried_sectors = []
				next_sector = (sector + 10 + @interleave) % @track_length[track]
			else
				next_sector = (next_sector + 1) % @track_length[track]
			end
		end
	end
...
class D81_image < Disk_image
	...
	def initialize(args)
		...
		if load
			...
			if @contents.length != 819200
				raise "Disk image '#{diskimage_filename}' isn't the correct length (should be 819200 bytes)"
			end
...
#		puts "Free blocks: #{diskimage_filename}..." if $verbose
	...
	def verify_bam_for_track(track)
		sect = track > 40 ? 2 : 1
		offset = (track > 40 ? (track - 41) : track - 1) * 6 + 16  
		pos = (@track_offset[40] + sect) * 256 + offset
		free = @contents[pos].ord
		c = 0
		5.times do |j|
			byte = @contents[pos + j + 1]
	...
	def set_id(ascii_id)
		...
		@contents[id_start, 2] = id
		id_start = (@track_offset[40] + 1) * 256 + 0x04
		@contents[id_start, 2] = id
		id_start = (@track_offset[40] + 2) * 256 + 0x04
		@contents[id_start, 2] = id
	end
	...
	def get_dos
		dos_start = @track_offset[40] * 256 + 0x19
		petscii_to_ascii(@contents[dos_start, 2])
	end

	def get_free_blocks
		sectpos = (@track_offset[@dir_start[0]] + 1) * 256 + 16
		free = 0
		40.times do |i|
			free += @contents[sectpos + 6 * i] if i != @dir_start[0] - 1
...
def print_usage
	...
	puts "  disk_image_1: New or existing file, e.g. disk.d81 or disk.d64"
	...
	puts"     for MEGA65 is used when writing files to d81 disk images."
	puts "\nSample commands:"
...
$disk_images.each do |img|
	if File.exist? img['filename']
		if img['filename'] =~ /\.d81$/i
			...
			puts "ERROR: Unknown file extension for disk image file #{img['filename']}. " +
				"Must be d64 or d81"
			exit 1
		end
	else
			...
			if src == 'f'
				...
				arg.each do |filepath|
					...
					if img['image'].find_file(filename16)
						print "ERROR: File exists in target disk image!"
					else
						img['image'].add_file(
							filename16, 
							data, 
